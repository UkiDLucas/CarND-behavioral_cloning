...
from googlenet_custom_layers import PoolHelper,LRN # googlenet_custom_layers.py
...
if __name__ == "__main__":
    img = imresize(imread(data_dir+ 'cat.jpeg', mode='RGB'), (224, 224)).astype(np.float32)
    img[:, :, 0] -= 123.68
    img[:, :, 1] -= 116.779
    img[:, :, 2] -= 103.939
    img[:,:,[0,1,2]] = img[:,:,[2,1,0]]
    ...
    model = create_googlenet(data_dir+ 'googlenet_weights.h5')
